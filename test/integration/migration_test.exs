defmodule Ecto.Integration.MigrationTest do
  ...
  defmodule AddColumnMigration do
    ...
    def up do
      ...
      execute("CREATE (:add_col_migration {value: 1, to_be_added: 2})")
    end
  ...
  defmodule AlterColumnMigration do
    ...
    def up do
      ...
      execute("CREATE (:alter_col_migration {from_null_to_not_null: 'foo'})")
    end
  ...
  defmodule RenameMigration do
    ...
    def up do
      ...
      execute("CREATE (:posts_migration)")
  ...
  test "create and drop table and indexes", %{migration_number: num} do
    ...
    CALL db.constraints()
    YIELD description
    WHERE description = "CONSTRAINT ON ( create_table_migration:create_table_migration ) ASSERT create_table_migration.value IS UNIQUE"
    RETURN COUNT(description) AS nb_constraints
    """
  ...
  test "correctly infers how to drop index", %{migration_number: num} do
    ...
    CALL db.indexes()
    YIELD description
    WHERE description = "INDEX ON :posts(title)"
    RETURN COUNT(description) AS nb_index
    """
  ...
  test "modify column", %{migration_number: num} do
    ...
    # Default is not supported by Neo4j
    # assert [nil] ==
    ...
    query = "CREATE (:alter_col_migration {from_not_null_to_null: 'foo'})"
  ...
  test "remove column", %{migration_number: num} do
    ...
    assert [nil] == PoolRepo.all(from p in "drop_col_migration", select: p.to_be_removed)
    # This does not produce an error in Neo4j
